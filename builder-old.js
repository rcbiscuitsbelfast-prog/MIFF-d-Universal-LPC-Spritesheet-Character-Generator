...
async function loadSprite(gender) {
  return new Promise((resolve, reject) => {
    ...
    const possiblePaths = [
      `/spritesheets/body/bodies/${gender}/spellcast/${bodyType.color}.png`,
      `/spritesheets/body/bodies/${gender}/walk/${bodyType.color}.png`,
      `/spritesheets/body/bodies/${gender}/hurt/${bodyType.color}.png`,
    ];
    ...
    const tryNextPath = () => {
      if (attemptIndex >= possiblePaths.length) {
        console.error('Failed all paths');
        showToast('Failed to load sprite');
        reject(new Error('All paths failed'));
        return;
      }
      
      const spritePath = possiblePaths[attemptIndex];
      console.log(`Try ${attemptIndex + 1}: ${spritePath}`);
